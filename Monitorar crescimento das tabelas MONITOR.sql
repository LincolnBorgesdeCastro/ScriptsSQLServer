...
ALTER AUTHORIZATION ON DATABASE::[MONITOR] TO [IPASGO\81084056199]
GO

ALTER DATABASE [Monitor] MODIFY FILE ( NAME = N'MONITOR_log', FILEGROWTH = 10240KB )
GO

ALTER DATABASE [Monitor] SET RECOVERY SIMPLE WITH ROLLBACK IMMEDIATE
GO
...
/*
-- Dropar as tabelas para o caso de recriar
...
if object_id('BaseDados') is null	
...
if object_id('Historico_Tamanho_Tabela') is null
...
as
	...
	select name
	from sys.databases
	where name not in ('master','model','tempdb','distribution', 'SBD', 'DNE', '') 
...
where  Nm_Database = 'IPASGO'
and     Nm_Tabela like '%scbd%'
