...
*******************************************************************************************************************************/
--	1) Fazer um Replace(CTRL + H) no caminho "E:\Dados" e alterar para um Caminho Real no servidor.
...
*******************************************************************************************************************************/
exec @rc = sp_trace_create @TraceID output, 0, N'E:\Dados\Duracao', @maxfilesize, NULL 
...
EndTime, Reads,Writes, CPU, ServerName, DataBaseName, RowCounts, SessionLoginName
FROM :: fn_trace_gettable(''E:\Dados\Duracao.trc'', default)
where Duration is not null
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
			...
			@os_run_priority = 0,
			@subsystem=N'CmdExec',
			@command = N'Del "E:\Dados\Duracao.trc" /Q',
			@flags = 0
