...
BEGIN
	CREATE DATABASE [Traces] 
		ON  PRIMARY ( 
			NAME = N'Traces', FILENAME = N'E:\Dados\Traces.mdf' , 
			SIZE = 500 MB , FILEGROWTH = 500 MB 
		)
		LOG ON ( 
			NAME = N'Traces_log', FILENAME = N'E:\Logs\Traces_log.ldf' , 
...
BEGIN
	EXEC [msdb].[dbo].[sp_add_operator]
		...
		@enabled = 1,
		@pager_days = 0,
		@email_address = N'lborges@ipasgo.go.gov.br'	-- Para colocar mais destinatarios, basta separar os e-mails com ponto e vírgula ";"
END
-- EXEC [msdb].[dbo].[sp_delete_operator] 'Alerta_BD'
GO
...
-- Tamanho das tabelas.
select Nm_Database,Nm_Tabela,Dt_Referencia
from Traces..vwTamanho_Tabela
WHERE Dt_Referencia = CAST(floor(CAST( GETDATE() AS FLOAT)) AS DATETIME)
group by Nm_Database,Nm_Tabela
order by 3 DESC

--select * from Historico_Utilizacao_Arquivo
--------------------------------------------------------------------------------------------------------------------------------
