...
-- Exemplo de execução
--====================
-- Salva todos os scripts de documentação no caminho \\bulgaria\SQL_BKP\
--		EXEC AdventureWorks..usp_docservidor '\\bulgaria\SQL_BKP\' 
-- Executada documentação apenas dos logins
--		EXEC AdventureWorks..usp_doclogins '\\bulgaria\SQL_BKP\' 
-- Executada documentação apenas dos usuários existentes nos DBs
--		EXEC AdventureWorks..usp_docusers '\\bulgaria\SQL_BKP\' 
...
begin	
	RAISERROR ('Ocorreu erro na documentação dos Logins',16, 1)WITH LOG
	goto erro
end
sucesso:    	
return(0)
...
print '3.3. Verifica se ocorreu erro no BCP dos usuários'
	...
	begin	
		...
		if @err = 0	
		begin
 		 	RAISERROR ('Ocorreu erro no BCP da documentação dos Usuários',16, 1)WITH LOG
  			goto erro
		end
		else
		begin
	...
	begin	
	 	RAISERROR ('Ocorreu erro na documentação dos Usuários',16, 1)WITH LOG
		goto erro
	end
sucesso:    	
drop table #tbAux
...
PRINT '4.3. Verifica se ocorreu erro no BCP dos Databases'
	...
	BEGIN
		...
		IF @err = 0	
			BEGIN
 		 		RAISERROR ('Ocorreu erro no BCP da documentação dos Databases',16, 1)WITH LOG
				SET @bco = 1
			END
		ELSE
			BEGIN --Sucesso 		
	...
	ELSE
	BEGIN
	 	RAISERROR ('Ocorreu erro na documentação dos Databases',16, 1)WITH LOG
		SET @bco = 1
	END
...
print '4.6. Verifica se ocorreu erro no BCP dos Default Databases'
	...
	begin			
	 	RAISERROR ('Ocorreu erro na documentação dos Default Databases',16, 1)WITH LOG
		set @def = 1
	end
...
PRINT '5.3. Verifica se ocorreu erro no BCP dos Devices de Backup'
	...
	BEGIN
		...
		IF @err = 0	
		BEGIN
 		 	RAISERROR ('Ocorreu erro no BCP da documentação dos Devices de Backup',16, 1)WITH LOG
  			GOTO erro
		END
	END
	ELSE
	BEGIN
	 	RAISERROR ('Ocorreu erro na documentação dos Device de Backup',16, 1)WITH LOG
		GOTO erro
	END
RETURN(0)
erro:
...
print '6.3. Verificando se ocorreu ERRO no BCP dos Jobs'
	...
	begin	
	 	RAISERROR ('Ocorreu erro na documentação dos Jobs',16, 1)WITH LOG
		goto erro
	end
sucesso:    	
return(0)
